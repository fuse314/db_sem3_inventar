...
where 
  (`n`.`ID_Device` = `d`.`ID_Device`)),0)),0) / sum(ifnull(`dt`.`NumInterfaces`,0))) * 100) AS `Usage`
  
from ((`usr_web98_5`.`location` `l` 
  join `usr_web98_5`.`device` `d` 
    on((`l`.`ID_Location` = `d`.`ID_Location`))) 
  join `usr_web98_5`.`devicetype` `dt` 
    on((`d`.`ID_DeviceType` = `dt`.`ID_DeviceType`))) group by `dt`.`ID_DeviceType`, `l`.`ID_Location`;
